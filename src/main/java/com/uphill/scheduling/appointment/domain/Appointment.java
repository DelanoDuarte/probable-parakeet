 ...
 * aggregate exists, an available doctor and room for the requested specialty and
 * slot have been chosen. The aggregate therefore guards its own internal
 * invariants (non-null collaborators, valid slot, immutable identity); the
 * cross-aggregate "no double booking" rule is enforced at the database level via
 * the unique constraints declared below, which is the only race-safe place to do
 * it under concurrency.
 *
 * <p>Doctor and room are referenced by id (with a denormalised display name copied
 * at booking time) rather than by object reference: they live in other bounded
 * contexts, so we reference across the boundary by identity, not by association.
 */
@Entity
@Table(
        name = "appointments",
...
public class Appointment {
    ...
    /**
     * Factory for a confirmed booking. All collaborators must be present —
     * an appointment cannot exist without a patient, a slot, a doctor and a room.
     */
    public static Appointment book(PatientInfo patient, Specialty specialty, TimeSlot slot,
        ...
        return new Appointment(UUID.randomUUID(), patient, specialty, slot,
                doctorId, doctorName, roomId, roomName);
    }
